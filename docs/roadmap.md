...
Reminders, Stripe Connect card payments, season awards, public previews of shared links, referral tracking and the founder dashboard are built. Also shipped: account merge for guests and calendar export, a venue address finder on the session form (Google Places or OpenStreetMap), and golf course cards: a shared library of par and stroke index per course and tee, seeded from a course-data API, a scanned photo of the paper card, or typed in, and corrected by whoever plays there next. Also shipped: the club hub for football and padel crews, covering leagues and cups, fixtures against an opponent, results with per-player goals, assists and manager ratings, league tables shared per division so only the first crew in a league ever sources one, kept live by the league's own feed snippet where a crew can get one, and season player stats. Also shipped: invite-only sessions (the organiser picks who can see and tap into a session; hidden from everyone else, off the shared calendar and link previews, reminders only to the invited), editing played sessions' details, and deleting a session outright (blocked while payments are recorded against it). Remaining from the original v1.1 list: a venue directory.

## Next: per-sport redesigns, then sign-in

Each sport gets its own look and dashboard, one at a time. Golf is in progress (so far: course banner, paper scorecard, clubhouse leader board, scores against par, points explained, a rounds-only feed). Football follows, then padel, gym and race weekends.

Last in this block, sign-in hardening:

- **Passkeys** (WebAuthn): saved to iCloud Keychain or Google Password Manager, sign in with Face ID or a fingerprint. The email code stays as the fallback for a new device.
- **Email and password login**: optional, hashed with a memory-hard algorithm, marked up (`autocomplete="username"`, `new-password`, `current-password`) so Apple and Google offer to save it, reset by email code, rate-limited.
- Nudge invite-link guests to add an email or passkey after their first session, so a lost phone does not lose their history.
