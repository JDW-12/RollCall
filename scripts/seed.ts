/**
 * Seeds a demo crew so the product can be seen at rest: "Tuesday FC" with 14 weeks of history,
 * plus a padel crew with one upcoming match. Run with `npm run db:seed`.
 *
 * Sign in as the organiser with the seeded session cookie printed at the end, or use the invite links.
 */
import { eq } from "drizzle-orm";
import { getDb, schema } from "../src/db/client";
import { newId, newToken, hueFrom, slugify } from "../src/lib/ids";
import { settleSession } from "../src/domain/money";
...
async function main() {
  ...
  if (existing.length) {
    console.log("Seed already present (tuesday-fc). Run `npm run db:reset` to start again.");
    return;
  }
  const now = new Date();
  ...
  console.log("Seeded.");
  console.log("  Crew:        /crew/tuesday-fc  (organiser: Sam, sam@example.com)");
  console.log("  Invite link: /join/tuesdayfc-demo-invite");
  console.log("  Padel crew:  /crew/battersea-padel (organiser: Priya)");
  console.log("  Dev sign-in: email sam@example.com, code printed to console.");
  console.log(`  Or set cookie rc_session=${token} to be Sam, rc_session=${priyaToken} to be Priya.`);
}
