...
import { listCompetitions, listMembers, venueHistory } from "@/lib/queries";
...
export default async function NewSessionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const others = (await listMembers(crew.id)).filter((m) => m.id !== user.id).map(({ id, name, hue }) => ({ id, name, hue }));
  return (
    <CrewShell crew={crew} user={user} active="sessions">
      ...
      <ActionForm action={createSession} className="max-w-md" marker="session-form">
        ...
        <SessionFields defaultSport={isSportKey(crew.sport) ? crew.sport : "football"} crewLateDropHours={crew.lateDropHours} venues={venueSuggestions(isSportKey(crew.sport) ? crew.sport : "football", await venueHistory(crew.id))} competitions={await listCompetitions(crew.id)} members={others} />
